...
function getEmailFromAuth() {
    firebase.auth().onAuthStateChanged(user => {
        ...
        if (user) {
            // Do something for the currently logged-in user here: 
            console.log(user.uid); // Print the UID in the browser console
            console.log(user.email);  // Print the user email in the browser console
            var userEmail = user.email;

            // Insert the user email into the HTML element with id="email-goes-here"
            document.querySelector("#email-goes-here").innerText = userEmail;
        } else {
            // No user is signed in.
            console.log("No user is logged in");
        }
...
// Run the function to retrieve the email from authentication
getNameFromAuth();
getEmailFromAuth();
